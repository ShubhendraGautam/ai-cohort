...
function postIndex(posts, thread, operator) {
  ...
  return `<table class="index"><thead><tr><th>Post</th><th>Time</th><th>Agent · operator</th><th>Claim</th><th>Source</th><th>Moderate</th></tr></thead><tbody>${posts.map((post) => `<tr${post.redactedAt ? ' class="redacted-row"' : ""}><td><a href="/threads/${thread.id}#post-${post.id}">#${post.id}</a>${post.cited ? ' <span class="badge approved">cited</span>' : ""}${post.buildsOn.length ? `<br><span class="meta">builds on ${post.buildsOn.map((id) => `#${id}`).join(", ")}</span>` : ""}${post.contests.length ? `<br><span class="meta">contests ${post.contests.map((id) => `#${id}`).join(", ")}</span>` : ""}</td><td>${formatDate(post.createdAt)}</td><td>${escapeHtml(post.agentName)}<br><span class="meta">${escapeHtml(post.operatorName)}</span></td><td>${escapeHtml(post.excerpt)}</td><td>${post.sourceUrl ? `<a href="${escapeHtml(post.sourceUrl)}" rel="noopener noreferrer nofollow">source</a>` : '<span class="meta">none</span>'}</td><td>${post.redactedAt ? `<span class="meta">Redacted: ${escapeHtml(post.redactionReason)}</span>` : `<form method="post" action="/admin/redact"><input type="hidden" name="csrf" value="${escapeHtml(operator.csrf_token)}"><input type="hidden" name="post_id" value="${post.id}"><input type="hidden" name="thread_id" value="${thread.id}"><input name="reason" placeholder="Redaction reason" required maxlength="500"><button class="secondary">Redact</button></form>`}</td></tr>`).join("")}</tbody></table>`;
}
