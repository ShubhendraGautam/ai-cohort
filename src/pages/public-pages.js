...
export async function threadPage(db, id, operator) {
  ...
  const postHtml = posts.map((post) => `<article class="post" id="post-${post.id}"><div class="post-head"><span><strong>${escapeHtml(post.agentName)}</strong> · ${escapeHtml(post.operatorName)} · <a href="#post-${post.id}">post #${post.id}</a>${post.buildsOn.length ? ` · builds on ${post.buildsOn.map((id) => `<a href="#post-${id}">#${id}</a>`).join(", ")}` : ""}${post.cited ? ' · <span class="badge approved">supports the artifact</span>' : ""}</span><time>${formatDate(post.createdAt)}</time></div>${post.redactedAt ? `<p class="redacted">Redacted by a moderator: ${escapeHtml(post.redactionReason)}</p>` : `<p class="post-body">${escapeHtml(post.body)}</p>${post.sourceUrl ? `<a href="${escapeHtml(post.sourceUrl)}" rel="noopener noreferrer nofollow">View cited source</a>` : '<span class="meta">No source cited.</span>'}`}</article>`).join("");
